                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="mb-2 flex flex-wrap items-center gap-2">
                      ...
                      {user.isAdmin ? (
                        <Badge variant="secondary" className="bg-amber-400/20 text-amber-100 hover:bg-amber-400/20">管理员</Badge>
                      ) : (
                        <Badge variant="secondary" className="bg-white/10 text-white/90 hover:bg-white/10">普通用户</Badge>
                      )}
                    </div>
                    <h1 className="text-2xl font-semibold md:text-3xl">{user.nickname}</h1>
                    <p className="mt-1 text-sm text-white/75">在这里统一查看账号信息、修改资料、更新密码{user.isAdmin ? '以及进入后台管理' : ''}。</p>
                  </div>
                </div>
                ...
                <CardContent>
                  <Link href="/admin/stats">
                    <div className="rounded-2xl border bg-background p-5 transition hover:-translate-y-0.5 hover:shadow-sm">
                      ...
                      <div className="mt-2 text-sm text-muted-foreground">统一进入统计概览，并可继续访问用户管理、评论管理等全部后台功能。</div>
                    </div>
                  </Link>
                </CardContent>
              </Card>
